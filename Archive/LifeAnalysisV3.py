...
JournalDates = BooleanDF.index[BooleanDF['Journal'] == 1].tolist()

Dummy = []
for i in range(0, len(JournalDates) - 1):
    diff = (JournalDates[i+1][0] - JournalDates[i][0]).days
    if diff != 1:
        Dummy.append([JournalDates[i][0], JournalDates[i+1][0]])

first = JournalDates[0][0]
last = JournalDates[len(JournalDates) - 1][0]
Ranges = []
for l1 in Dummy:
    for l2 in l1:
        Ranges.append(l2)
...
for x, y in zip(AllStart, AllEnd):
    print(x, y)

# start = JournalDates[0][0]
# end = JournalDates[1][0]
# Ranges = []
#
# for i in range(0, len(JournalDates) - 1):
#     diff = (JournalDates[i+1][0] - JournalDates[i][0]).days
#     if diff != 1:
#         end = JournalDates[i][0]
#         Ranges.append([start, end])
#         start = JournalDates[i+1][0]
#
#     # Manually account for missing end values
#     if i == len(JournalDates) - 2:
#         if (JournalDates[i+1][0] - JournalDates[i][0]).days == 1:
#             Ranges.append([JournalDates[i][0], JournalDates[i+1][0]])
#         else:
#             Ranges.append([JournalDates[i+1][0], JournalDates[i+1][0]])
#             Ranges.append([JournalDates[i][0], JournalDates[i][0]])


# AllStart = [item[0] for item in Ranges]
# AllEnd = [item[1] for item in Ranges]
#
JournalDF = pd.DataFrame({'Start' : AllStart, 'End' : AllEnd})
JournalDF.insert(0, "Variable", "Journal")

# Bar graph cannot represent single date values so have to add 1
JournalDF["End"] = JournalDF["End"] + pd.Timedelta(days = 1)
print(JournalDF)

fig = px.timeline(JournalDF, x_start = "Start", x_end = "End", y = "Variable")
fig.show()
